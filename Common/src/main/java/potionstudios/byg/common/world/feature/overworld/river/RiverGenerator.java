...
public class RiverGenerator {
    ...
    public RiverGenerator(FastNoise noise, WorldGenLevel world, BlockPos startPos, ChunkGenerator generator, Predicate<BlockPos> isInvalid, Predicate<BlockPos> isValid, int maxDistance) {
        ...
        for (int i = 1; i < distanceInNodes; i++) {
            ...
            ResourceKey<Biome> biomeRegistryKey = world.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getResourceKey(generator.getBiomeSource().getNoiseBiome(addedPos.getX() >> 2, addedPos.getY() >> 2, addedPos.getZ() >> 2, generator.climateSampler())).get();
            int newY = /*biomeRegistryKey == BYGBiomes.CANYON_KEY ? 218 :*/ generator.getFirstFreeHeight(addedPos.getX(), addedPos.getZ(), Heightmap.Types.OCEAN_FLOOR_WG, world);
