...
public class MegaChunk {
    ...
    public MegaChunk(MegaChunkPos megaChunkPos, ChunkGenerator chunkGenerator, Set<Biome> allowedBiomes) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                Biome noiseBiome = chunkGenerator.getBiomeSource().getNoiseBiome((chunkPos.x << 2) + 2, 0, (chunkPos.z << 2) + 2, chunkGenerator.climateSampler());
    ...
    public void createRiverGeneratorStart(FastNoise noise, WorldGenLevel world, long worldSeed, int maxRiverDistance) {
        WorldgenRandom seedRandom = new WorldgenRandom(new XoroshiroRandomSource(worldSeed));
        ...
        this.riverGenerator = new RiverGenerator(noise, world, new BlockPos(startPos.getX(), this.chunkGenerator.getFirstFreeHeight(startPos.getX(), startPos.getZ(), Heightmap.Types.OCEAN_FLOOR_WG, world) /*218*/, startPos.getZ()), this.chunkGenerator, blockpos -> false, blockpos -> {
            Biome.BiomeCategory category = this.provider.getNoiseBiome(blockpos.getX() >> 2, blockpos.getY() >> 2, blockpos.getZ() >> 2, chunkGenerator.climateSampler()).getBiomeCategory();
