...
public class WarpedDesert extends Biome implements BiomeFog {
    ...
    public WarpedDesert() {
        super(new Settings().surfaceBuilder(SURFACE_BUILDER).precipitation(PRECIPATATION).category(CATEGORY).depth((float) DEPTH).scale((float) SCALE).temperature(TEMPERATURE).downfall(DOWNFALL).parent(PARENT).effects((new BiomeEffects.Builder()).waterColor(WATER_COLOR).waterFogColor(WATER_FOG_COLOR).fogColor(12638463).moodSound(BiomeMoodSound.CAVE).build()).noises(ImmutableList.of(new Biome.MixedNoisePoint(0.4F, 0.0F, 0.0F, 0.0F, 0.8F))));
        ...
        this.addSpawn(SpawnGroup.MONSTER, new SpawnEntry(EntityType.GHAST, 60, 4, 4));
    ...
    private static final Vec3d WARPEDDESERTVEC3D = new Vec3d(0.53F, 0.812F, 0.941F);
}
