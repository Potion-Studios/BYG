...
public class MiniBrownMushroom extends BYGAbstractMushroomFeature<BYGMushroomFeatureConfig> {

    public MiniBrownMushroom(Codec<BYGMushroomFeatureConfig> configIn) {
        super(configIn);
    }
    ...
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, BYGMushroomFeatureConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getHeight()) {
            ...
            if (worldIn.getBlockState(pos.down()).getBlock() != Blocks.GRASS_BLOCK && worldIn.getBlockState(pos.down()).getBlock() != Blocks.SOUL_SAND && worldIn.getBlockState(pos.down()).getBlock() != BYGBlockList.OVERGROWN_NETHERRACK) {
                ...
                this.leafs(worldIn, mainmutable.add(1, 2, -1));
                this.leafs(worldIn, mainmutable.add(-1, 2, 0));
    ...
    private void leafs(Set<BlockPos> blockPos, IWorldGenerationReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        if (isAir(reader, pos)) {
            this.setFinalBlockState(blockPos, reader, pos, Blocks.BROWN_MUSHROOM_BLOCK.getDefaultState(), boundingBox);
        }
    }
