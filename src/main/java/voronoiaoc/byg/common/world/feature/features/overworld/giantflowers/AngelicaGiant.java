...
public class AngelicaGiant extends BYGAbstractTreeFeature<NoneFeatureConfiguration> {

    public AngelicaGiant(Codec<NoneFeatureConfiguration> configIn) {
        super(configIn);
    }
    ...
    protected boolean place(Set<BlockPos> changedBlocks, WorldGenLevel worldIn, Random rand, BlockPos pos, BoundingBox boundsIn, boolean isSapling) {
        ...
        if (pos.getY() + randTreeHeight + 1 < 256) {
            ...
            if (!isDesiredGroundwDirtTag(worldIn, pos.below(), Blocks.GRASS_BLOCK)) {
                ...
                this.treeBranch(changedBlocks, worldIn, mainmutable.offset(3, 5, -2), boundsIn);
                ...
                this.leafs(changedBlocks, worldIn, mainmutable.offset(1, 11, 0), boundsIn);
                this.leafs(changedBlocks, worldIn, mainmutable.offset(0, 11, 2), boundsIn);
                this.pollen(changedBlocks, worldIn, mainmutable.offset(-1, 11, 1), boundsIn);
                this.pollen(changedBlocks, worldIn, mainmutable.offset(0, 11, 1), boundsIn);
                ...
                this.leafs(changedBlocks, worldIn, mainmutable.offset(0, 12, 2), boundsIn);
                this.pollen(changedBlocks, worldIn, mainmutable.offset(0, 12, -1), boundsIn);
                ...
                this.leafs(changedBlocks, worldIn, mainmutable.offset(1, 13, 0), boundsIn);
                this.leafs(changedBlocks, worldIn, mainmutable.offset(0, 13, 1), boundsIn);
                this.pollen(changedBlocks, worldIn, mainmutable.offset(0, 13, 0), boundsIn);
                this.leafs(changedBlocks, worldIn, mainmutable.offset(0, 14, -1), boundsIn);
                this.leafs(changedBlocks, worldIn, mainmutable.offset(0, 14, 0), boundsIn);
                this.leafs(changedBlocks, worldIn, mainmutable.offset(0, 14, 1), boundsIn);
            }
        }
        return true;
    ...
    private void leafs(Set<BlockPos> blockPos, WorldGenLevel reader, BlockPos pos, BoundingBox boundingBox) {
        ...
        if (isAir(reader, blockpos)) {
            this.setFinalBlockState(blockPos, reader, blockpos, BYGBlockList.WHITE_PETAL.defaultBlockState(), boundingBox);
        }
    }
