...
public class WarpedCoral extends Feature<NoneFeatureConfiguration> {
    public WarpedCoral(Codec<NoneFeatureConfiguration> config) {
        super(config);
    }
    ...
    public boolean place(WorldGenLevel worldIn, ChunkGenerator generator, Random random, BlockPos pos, NoneFeatureConfiguration config) {
        if (!worldIn.isEmptyBlock(pos)) {
            ...
            worldIn.setBlock(pos, BYGBlockList.WARPED_CORAL.defaultBlockState(), 2);
            return true;
        }
        return true;
    }
