...
public class WeepingMilkCapMushroom extends Feature<NoneFeatureConfiguration> {
    public WeepingMilkCapMushroom(Codec<NoneFeatureConfiguration> config) {
        super(config);
    }
    ...
    public boolean place(WorldGenLevel worldIn, ChunkGenerator generator, Random random, BlockPos pos, NoneFeatureConfiguration config) {
        if (!worldIn.isEmptyBlock(pos) || worldIn.getBlockState(pos.below()).getBlock() != BYGBlockList.OVERGROWN_NETHERRACK) {
            ...
            worldIn.setBlock(pos, BYGBlockList.WEEPING_MILKCAP.defaultBlockState(), 10);
            return true;
        }
    }
}