...
public class OrchardTree1 extends BYGAbstractTreeFeature<NoneFeatureConfiguration> {

    public OrchardTree1(Codec<NoneFeatureConfiguration> configIn) {
        super(configIn);
    }
    ...
    protected boolean place(Set<BlockPos> changedBlocks, WorldGenLevel worldIn, Random rand, BlockPos pos, BoundingBox boundsIn, boolean isSapling) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getHeight()) {
            ...
            if (!isDesiredGroundwDirtTag(worldIn, pos.below(), Blocks.GRASS_BLOCK)) {
                ...
                this.treeBranch(changedBlocks, worldIn, mainmutable.offset(-5, randTreeHeight + 1, 1), boundsIn);
    ...
    public BlockState randomLeaves() {
        ...
        if (randomizer == 1)
            return BYGBlockList.FLOWERING_ORCHARD_LEAVES.defaultBlockState();
        else if (randomizer == 0)
            return BYGBlockList.RIPE_ORCHARD_LEAVES.defaultBlockState();
        else
            return BYGBlockList.ORCHARD_LEAVES.defaultBlockState();
    }
