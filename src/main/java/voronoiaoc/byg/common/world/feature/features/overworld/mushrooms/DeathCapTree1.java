    ...
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, BYGMushroomFeatureConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getHeight()) {
            ...
            if (!isDesiredGroundwDirtTag(worldIn, blockpos, BYGBlockList.SYTHIAN_NYLIUM)) {
                ...
                this.leafs(worldIn, mainmutable.add(-2, 5, 0));
                this.leafs(worldIn, mainmutable.add(-2, 5, 1));
                ...
                this.leafs(worldIn, mainmutable.add(2, 5, 0));
                this.leafs(worldIn, mainmutable.add(2, 5, 1));
                this.leafs(worldIn, mainmutable.add(-3, 6, -2));
                this.leafs(worldIn, mainmutable.add(-3, 6, -1));
                this.leafs(worldIn, mainmutable.add(-3, 6, 0));
                this.leafs(worldIn, mainmutable.add(-3, 6, 1));
                this.leafs(worldIn, mainmutable.add(-3, 6, 2));
                this.leafs(worldIn, mainmutable.add(-2, 6, -3));
                this.leafs(worldIn, mainmutable.add(-2, 6, -2));
                this.leafs(worldIn, mainmutable.add(-2, 6, 2));
                this.leafs(worldIn, mainmutable.add(-2, 6, 3));
                this.leafs(worldIn, mainmutable.add(-1, 6, -3));
                this.leafs(worldIn, mainmutable.add(-1, 6, 3));
