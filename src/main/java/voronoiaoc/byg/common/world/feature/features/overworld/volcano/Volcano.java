...
public class Volcano extends Feature<NoFeatureConfig> {
    ...
    public Volcano(Codec<NoFeatureConfig> config) {
        super(config);
    }
    ...
    public boolean func_241855_a(ISeedReader world, ChunkGenerator generator, Random rand, BlockPos position, NoFeatureConfig config) {
        {
            ...
            for (int x = 0; x < 16; x++) {
                for (int z = 0; z < 16; z++) {
                    ...
                    noise = Math.pow(Math.abs(noiseGen.sample2D(mutable.getX() * 0.003D, mutable.getZ() * 0.003D)) + noise2 * 0.005D, 7); //0.70990733195111407153665966708847
