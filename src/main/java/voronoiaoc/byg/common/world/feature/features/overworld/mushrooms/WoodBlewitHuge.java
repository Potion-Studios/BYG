...
public class WoodBlewitHuge extends BYGAbstractMushroomFeature<BYGMushroomFeatureConfig> {
    ...
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, BYGMushroomFeatureConfig config) {
        ...
        BlockState POLLEN = config.getPollenProvider().getBlockState(rand, pos);int randTreeHeight = 8 + rand.nextInt(5);
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getHeight()) {
            ...
            if (!isDesiredGroundwDirtTag(worldIn, blockpos, Blocks.GRASS_BLOCK)) {
                ...
                this.leafs(worldIn, mainmutable.add(0, 5, -3));
                this.leafs(worldIn, mainmutable.add(-2, 5, -2));
                this.leafs(worldIn, mainmutable.add(-1, 5, -2));
                this.leafs(worldIn, mainmutable.add(1, 5, -2));
                this.leafs(worldIn, mainmutable.add(2, 5, -2));
                this.leafs(worldIn, mainmutable.add(-3, 5, -1));
                this.leafs(worldIn, mainmutable.add(-2, 5, -1));
                this.leafs(worldIn, mainmutable.add(2, 5, -1));
                this.leafs(worldIn, mainmutable.add(3, 5, -1));
                this.leafs(worldIn, mainmutable.add(-3, 5, 0));
                this.leafs(worldIn, mainmutable.add(3, 5, 0));
                this.leafs(worldIn, mainmutable.add(-3, 5, 1));
                this.leafs(worldIn, mainmutable.add(-2, 5, 1));
                this.leafs(worldIn, mainmutable.add(2, 5, 1));
    ...
    private void leafs(Set<BlockPos> blockPos, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        if (isAir(reader, pos)) {
            this.setFinalBlockState(blockPos, reader, pos, BYGBlockList.BLEWIT_MUSHROOM_BLOCK.getDefaultState(), boundingBox);
        }
    }
