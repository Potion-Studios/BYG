...
public class ShatteredGlacierSB2 extends ShatteredGlacierSB {
    ...
    public void generate(Random random, Chunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, TernarySurfaceConfig config) {
        ...
        BlockState blockstateUnder = biomeIn.getSurfaceBuilder().get().getConfig().getUnderMaterial();
            ...
            if (blockstate1.isAir()) {
                ...
                if (j == -1) {
                    ...
                    if (i1 <= 0) {
                        ...
                        blockstateUnder = biomeIn.getSurfaceBuilder().get().getConfig().getUnderMaterial();
                    }
                    ...
                    if (k >= seaLevel - 1) {
                        if (k <= seaLevel + 3 + i1) {
                            chunkIn.setBlockState(blockpos$mutable, biomeIn.getSurfaceBuilder().get().getConfig().getUnderMaterial(), false);
                            flag1 = true;
                        } else {
