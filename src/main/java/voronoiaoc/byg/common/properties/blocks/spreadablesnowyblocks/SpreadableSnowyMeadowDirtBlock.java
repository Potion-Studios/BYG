...
public class SpreadableSnowyMeadowDirtBlock extends SnowyBlock {
    public SpreadableSnowyMeadowDirtBlock(Settings properties) {
        super(properties);
    }
        ...
        if (!canSurvive(state, worldIn, pos)) {
            ...
            if (worldIn.getBrightness(pos.up()) >= 9) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (worldIn.getBlockState(blockpos).getBlock() == BYGBlockList.MEADOW_DIRT && canSpread(blockstate, worldIn, blockpos)) {
