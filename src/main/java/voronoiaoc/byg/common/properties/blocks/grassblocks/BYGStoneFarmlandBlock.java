...
public class BYGStoneFarmlandBlock extends FarmlandBlock {
    ...
    protected BYGStoneFarmlandBlock(Block.Properties builder) {
        super(builder);
        this.setDefaultState(this.stateContainer.getBaseState().with(MOISTURE, Integer.valueOf(0)));
    }

    public static void turnToDirt(BlockState state, World worldIn, BlockPos pos) {
        worldIn.setBlockState(pos, nudgeEntitiesWithNewState(state, Blocks.STONE.getDefaultState(), worldIn, pos));
    }
    ...
    public BlockState getStateForPlacement(BlockItemUseContext context) {
        return !this.getDefaultState().isValidPosition(context.getWorld(), context.getPos()) ? Blocks.STONE.getDefaultState() : super.getStateForPlacement(context);
    }
