...
public class WorldConfig {
    ...
    public WorldConfig(Path path) {
        ...
        this.rockyStoneSpawns = stoneSubConfig.add("What biome(s) does Rocky Stone spawn in? Default: \"byg:shattered_viscal_isles\".", "rockyStoneSpawns", "byg:shattered_viscal_isles");
        this.scoriaSpawns = stoneSubConfig.add("What biome(s) does Scoria Stone spawn in? Default: \"byg#END\".", "scoriaStoneSpawns", "byg#END");
        this.soapStoneSpawns = stoneSubConfig.add("What biome(s) does Soap Stone spawn in? Default: \"byg:forest_fault\".", "soapStoneSpawns", "byg:forest_fault");
        this.configHelper.addSubConfig("BYG uses a prefix system for its stone spawns.\n Prefix Guide:\n \"#\" - Biome category representable.\n \"$\" - Biome dictionary representable.\n \",\" - Creates a new condition, separate from the previous.\n \"ALL\" - Spawn in all biomes(no condition).\n \"!\" - Negates/flips/does the reverse of the condition. MUST BE THE FIRST CHARACTER for the condition!\n \"\" - No prefix serves as a biome ID OR Mod ID representable.\n\n Here are a few examples:\n1. \"byg#END, $OCEAN\" would mean that the stone may spawn in biomes with the name space \"byg\" AND in the \"END\" biome category, OR all biomes in the \"OCEAN\" dictionary.\n2. \"byg:guiana_shield, #MESA\" would mean that the stone may spawn in the \"byg:guiana_shield\" OR all biomes in the \"MESA\" category.\n3. \"byg#ICY$MOUNTAIN\" would mean that the stone may only spawn in biomes from byg in the \"ICY\" category and \"MOUNTAIN\" dictionary type.\n4. \"!byg#DESERT\" would mean that the stone may only spawn in biomes that are NOT from byg and NOT in the \"DESERT\" category.\n5. \"ALL\", spawn everywhere. \n6. \"\" Don't spawn anywhere.", "stones", stoneSubConfig);
