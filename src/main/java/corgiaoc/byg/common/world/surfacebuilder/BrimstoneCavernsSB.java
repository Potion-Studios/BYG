...
public class BrimstoneCavernsSB extends SurfaceBuilder<SurfaceBuilderBaseConfiguration> {
    public BrimstoneCavernsSB(Codec<SurfaceBuilderBaseConfiguration> p_i51312_1_) {
        super(p_i51312_1_);
    }
    ...
    public void apply(Random random, ChunkAccess chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, SurfaceBuilderBaseConfiguration config) {
        ...
        for (int yPos = 256; yPos >= seaLevel; --yPos) {
            ...
            if (currentBlockToReplace == Blocks.NETHERRACK.defaultBlockState() && !(checkAirForSB.getBlock() == Blocks.AIR))
                chunkIn.setBlockState(block, BYGBlocks.BRIMSTONE.defaultBlockState(), false);
