...
public class VolcanicWastes extends SurfaceBuilder<TernarySurfaceConfig> {
    ...
    public void generate(Random random, Chunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, int i, long seed, TernarySurfaceConfig config) {
        if (noise > 0.3)
            ...
            SurfaceBuilder.DEFAULT.generate(random, chunkIn, biomeIn, x, z, startHeight, noise, defaultBlock, defaultFluid, seaLevel, i, seed, random.nextInt(4) == 0 ? new TernarySurfaceConfig(Blocks.MAGMA_BLOCK.getDefaultState(), BYGBlocks.MAGMATIC_STONE.getDefaultState(), BYGBlocks.MAGMATIC_STONE.getDefaultState()) : new TernarySurfaceConfig(BYGBlocks.MAGMATIC_STONE.getDefaultState(), Blocks.OBSIDIAN.getDefaultState(), Blocks.OBSIDIAN.getDefaultState()));
