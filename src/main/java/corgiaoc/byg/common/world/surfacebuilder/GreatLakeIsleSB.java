...
public class GreatLakeIsleSB extends SurfaceBuilder<TernarySurfaceConfig> {
    ...
    public void generate(Random random, Chunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, int i1, long seed, TernarySurfaceConfig config) {
        ...
        if (noise > 1.75D) {
            SurfaceBuilder.DEFAULT.generate(random, chunkIn, biomeIn, x, z, startHeight, noise, defaultBlock, defaultFluid, seaLevel, i1, seed, BYGSurfaceBuilders.Configs.PEATGRASS_CF);
        } else if (noise > -0.95D) {
