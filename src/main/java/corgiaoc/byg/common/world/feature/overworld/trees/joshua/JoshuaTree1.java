        ...
        if (posY + randTreeHeight + 1 < worldIn.getMaxBuildHeight()) {
            ...
            if (!isDesiredGroundwDirtTag(worldIn, pos.below(), config)) {
                ...
                for (int buildTrunk = 0; buildTrunk < randTreeHeight; ++buildTrunk) {
                    ...
                    placeTrunk(config, rand, changedBlocks, worldIn, blockpos2.north(2).below(), boundsIn);
                    placeTrunk(config, rand, changedBlocks, worldIn, blockpos2.south().below(), boundsIn);
                    placeTrunk(config, rand, changedBlocks, worldIn, blockpos2.south(), boundsIn);
                    placeTrunk(config, rand, changedBlocks, worldIn, blockpos2.east().below(), boundsIn);
                    placeTrunk(config, rand, changedBlocks, worldIn, blockpos2.east(), boundsIn);
                    placeTrunk(config, rand, changedBlocks, worldIn, blockpos2.west().below(), boundsIn);
                    placeTrunk(config, rand, changedBlocks, worldIn, blockpos2.west(2).below(), boundsIn);
