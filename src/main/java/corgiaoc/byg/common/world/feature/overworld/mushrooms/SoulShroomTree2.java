    ...
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, BYGMushroomFeatureConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getHeight()) {
            if (!isDesiredGroundwDirtTag(worldIn, pos.down(), Blocks.GRASS_BLOCK)) {
                ...
                this.soulShroomSporeEnd(worldIn, mainmutable.setPos(pos).move(-2, 3, 1));
                this.soulShroomSpore(worldIn, mainmutable.setPos(pos).move(-2, 4, 1));
                this.soulShroomSpore(worldIn, mainmutable.setPos(pos).move(-2, 5, 1));
                this.soulShroomSporeEnd(worldIn, mainmutable.setPos(pos).move(0, 5, -2));
                this.soulShroomSporeEnd(worldIn, mainmutable.setPos(pos).move(1, 5, 2));
