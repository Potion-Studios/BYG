...
public class BlueGlowshroomHuge extends BYGAbstractMushroomFeature<BYGMushroomConfig> {

    public BlueGlowshroomHuge(Codec<BYGMushroomConfig> configIn) {
        super(configIn);
    }
    ...
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, BYGMushroomConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getHeight()) {
            if (!isDesiredGroundwDirtTag(config, worldIn, pos.down(), Blocks.GRASS_BLOCK)) {
                ...
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(-2, 5, -3));
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(-1, 5, -3));
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(1, 5, -3));
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(2, 5, -3));
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(-3, 5, -2));
