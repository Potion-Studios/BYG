...
public class WoodBlewitHuge extends BYGAbstractMushroomFeature<BYGMushroomFeatureConfig> {

    public WoodBlewitHuge(Codec<BYGMushroomFeatureConfig> configIn) {
        super(configIn);
    }
    ...
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, BYGMushroomFeatureConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getHeight()) {
            if (!isDesiredGroundwDirtTag(worldIn, pos.down(), Blocks.GRASS_BLOCK)) {
                ...
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(2, 6, -1));
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(-3, 6, 0));
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(3, 6, 0));
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(-2, 6, 1));
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(2, 6, 1));
                placeMushroom(MUSHROOM, worldIn, mainmutable.setPos(pos).move(-1, 6, 2));
