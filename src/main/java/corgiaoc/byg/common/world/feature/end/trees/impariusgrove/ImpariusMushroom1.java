    ...
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, BYGMushroomConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getHeight()) {
            if (!isDesiredGroundwEndTags(config, worldIn, pos.down(), BYGBlocks.IMPARIUS_PHYLIUM)) {
                ...
                placeStemBranch(STEM, worldIn, mainmutable.setPos(pos).move(-2, 0, -1));
                ...
                this.vine(worldIn, mainmutable.setPos(pos).move(0, 1, 0));
                this.vine(worldIn, mainmutable.setPos(pos).move(-1, 2, -2));
                this.vineplant(worldIn, mainmutable.setPos(pos).move(0, 2, 0));
                this.vine(worldIn, mainmutable.setPos(pos).move(1, 2, -1));
                this.imparius_mushroom_branch(worldIn, mainmutable.setPos(pos).move(-1, 4, -1), Direction.WEST);
                this.imparius_mushroom_branch(worldIn, mainmutable.setPos(pos).move(0, 4, -2), Direction.NORTH);
                this.imparius_mushroom_branch(worldIn, mainmutable.setPos(pos).move(0, 4, 0), Direction.SOUTH);
                this.imparius_mushroom_branch(worldIn, mainmutable.setPos(pos).move(1, 4, -1), Direction.EAST);
            }
        }
        return true;
    }
