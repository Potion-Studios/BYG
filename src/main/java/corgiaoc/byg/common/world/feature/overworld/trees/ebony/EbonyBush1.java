    ...
    protected boolean generate(Set<BlockPos> changedBlocks, ISeedReader worldIn, Random rand, BlockPos pos, MutableBoundingBox boundsIn, boolean isSapling, BYGTreeFeatureConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getHeight()) {
            ...
            if (!isDesiredGroundwDirtTag(worldIn, pos.down(), config)) {
                ...
                return false;
            } else if (this.isCliff(worldIn, mainmutable.setPos(pos).move(0, 0, 0), mainmutable.setPos(pos).move(0, 0, -2), mainmutable.setPos(pos).move(-1, 0, -1), mainmutable.setPos(pos).move(0, 0, -1), mainmutable.setPos(pos).move(1, 0, -1), mainmutable.setPos(pos).move(-2, 0, 0), mainmutable.setPos(pos).move(-1, 0, 0), mainmutable.setPos(pos).move(1, 0, 0), mainmutable.setPos(pos).move(-1, 0, 1), mainmutable.setPos(pos).move(0, 0, 1))) {
                return false;
            } else {
