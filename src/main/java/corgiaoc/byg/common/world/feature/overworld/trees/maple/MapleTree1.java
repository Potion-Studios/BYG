...
public class MapleTree1 extends BYGAbstractTreeFeature<BYGTreeConfig> {
    ...
    protected boolean generate(Set<BlockPos> changedBlocks, ISeedReader worldIn, Random rand, BlockPos pos, MutableBoundingBox boundsIn, boolean isSapling, BYGTreeConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getMaxBuildHeight()) {
            ...
            if (!isDesiredGroundwDirtTag(worldIn, pos.below(), config)) {
                ...
                buildTrunkBase(pos, changedBlocks, worldIn, config, rand, boundsIn, mainmutable.set(pos).immutable(),  mainmutable.set(pos).move(0, 0, -1).immutable(), mainmutable.set(pos).move(-1, 0, 0).immutable(), mainmutable.set(pos).move(1, 0, 0).immutable(), mainmutable.set(pos).move(0, 0, 1));
                mainmutable.set(pos);
    ...
    private void leafs(Set<BlockPos> blockPos, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        ...
        if (isAir(reader, blockpos)) {
            this.setFinalBlockState(blockPos, reader, blockpos, BYGBlocks.MAPLE_LEAVES.defaultBlockState(), boundingBox);
        }
    }
